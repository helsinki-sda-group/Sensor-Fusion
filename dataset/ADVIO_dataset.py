...
from utils.utils import rotationError, read_pose_from_text, get_relative_pose_sequence
...
class ADVIO(Dataset):
    ...
    def make_dataset(self):
        ...
        for folder in self.train_seqs:
            poses = np.loadtxt(self.root/f'poses/{folder}.txt')[::6]
            imus = np.loadtxt(self.root/f'imus/{folder}.txt')
            timestamps = np.loadtxt(self.root/f"timestamps/{folder}.txt")[:,0][::6]
            
            png_paths = (self.root/'sequences/{}/images_2'.format(folder)).files("*.png")
            jpg_paths = (self.root/'sequences/{}/images_2'.format(folder)).files("*.jpg")
            fpaths = sorted(png_paths + jpg_paths) 
            
            for i in range(len(fpaths)-(self.sequence_length*N)):
                start, end = timestamps[[i, i+(self.sequence_length*N)]]
                img_samples = fpaths[i:i+(self.sequence_length*N):N]
                ...
                pose_rel_samples = get_relative_pose_sequence(pose_samples)
    ...
    def __getitem__(self, index):
        ...
        imgs = [np.transpose(np.asarray(Image.open(img)), (1,0,2)) for img in sample['imgs']]
