...
class KITTI(Dataset):
    ...
    def make_dataset(self):
        ...
        for folder in self.train_seqs:
            ...
            png_paths = (self.root/'sequences/{}/image_2'.format(folder)).files("*.png")
            jpg_paths = (self.root/'sequences/{}/image_2'.format(folder)).files("*.jpg")
            ...
            for i in range(len(fpaths)-(self.sequence_length*N)):
                ...
                imu_samples = imus[i*IMU_FREQ:(i+(self.sequence_length-1)*N)*IMU_FREQ+1]
                pose_samples = poses[i:i+(self.sequence_length*N):N]
                pose_rel_samples = get_relative_pose_sequence(pose_samples) #poses_rel[i:i+(self.sequence_length-1)*N:N]
