...
class TransformerLayer(nn.Module):
    ...
    def forward(self, source, target,
        ...
        if self.attention_type == 'full_attention' or attn_num_splits <= 1:
            message = single_head_full_attention(query, key, value)  # [B, L, C]

        elif self.attention_type == 'swin' and attn_num_splits > 1:
            if self.nhead > 1:
...
class FeatureFlowAttention(nn.Module):
    ...
    def forward(self, feature0, flow,
        ...
        if token is not None:
            value = torch.cat([token[0].expand(value.size(0), -1, -1), value], dim=1)
            query = torch.cat([token[1].expand(query.size(0), -1, -1), query], dim=1)
        ...
        out = torch.matmul(prob, value)  # [B, H*W, 2] or [B, H*W, C] if token
        if token is None:
...
class FeaturesFromFFA(nn.Module):
    """
    Extract a feature representation of flow propagation with self-attention on feature
    query: feature0, key: feature0, value: flow
    """
    ...
    def __init__(self, in_channels,
        ...
        super(FeaturesFromFFA, self).__init__()

        self.q_proj = nn.Linear(in_channels, in_channels)
        self.k_proj = nn.Linear(in_channels, in_channels)
        self.v_proj = nn.Linear(2, in_channels)
    ...
    def forward(self, feature0, flow,
        ...
        print(f"FFA X_value shape {flow.shape}")
        value = flow.view(b, flow.size(1), h * w).permute(0, 2, 1)  # [B, H*W, 2]
        print(f"FFA intermidiate shape {value.shape}")
        value = self.v_proj(value)
        print(f"FFA Value shape: {value.shape}")
