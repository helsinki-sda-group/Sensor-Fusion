...
class UniMatch(nn.Module):
    ...
    def inference(self,
        ...
        Returns: flow prediction (2 dim.), image features (128 dim.)
        """
        b = image1.shape[0]
        ...
        results_dict = self.forward(image1, image2,
                                attn_type=attn_type,
        ...
        #flow_raw = results_dict['flow_raw']

        if save_flow:
            b = min(b,11)
            for i in range(b):
                # save vis flow
                filename = './batch_' + str(i) + '.png'
                flow = flow_pr[i].permute(1, 2, 0).detach().cpu().numpy()  # [H/4, W/4, 2]
                save_vis_flow_tofile(flow, filename)
        
        #features = results_dict['features']
        
        return flow_pr #, flow_features