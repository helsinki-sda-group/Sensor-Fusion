...
class IMUNoise(object):
    """Injects small gaussian noise to the IMU with a probability of 0.5"""
    def __init__(self, p=0.5, mean=0, std=1e-5):
        self.p = p
        self.mean = mean
        self.std = std
    def __call__(self, images, imus, gts):
        if random.random() < self.p:
            imus = imus + np.random.normal(self.mean,self.std, imus.shape)
