...
#     def __init__(self, opt, folder, skip_frames=10):
...
#         png_paths = glob.glob('{}{}/image_2/*.png'.format(image_dir, self.folder))
#         jpg_paths = glob.glob('{}{}/image_2/*.jpg'.format(image_dir, self.folder))
#         self.img_paths = sorted(png_paths + jpg_paths)        
...
#         print(self.poses.shape, self.poses_rel.shape)
#         self.img_paths.sort()

#         self.img_paths_list, self.poses_list, self.imus_list = [], [], []
#         start = 0
#         n_frames = len(self.img_paths)
#         while start + (self.seq_len*N) < n_frames:
#             self.img_paths_list.append(self.img_paths[start:start + (self.seq_len*N):N])
#             self.poses_list.append(get_relative_pose_sequence(self.poses[start:start + (self.seq_len)*N:N]))
#             self.imus_list.append(self.imus[start * 10:(start + (self.seq_len-1)*N) * 10 + 1])
#             start += (self.seq_len - 1)*N
#         # if len(self.poses_rel[start::N]) > N:
#         #     self.img_paths_list.append(self.img_paths[start::N])
#         #     self.poses_list.append(get_relative_pose_sequence(self.poses[start:start + (self.seq_len)*N:N]))
#         #     self.imus_list.append(self.imus[start * 10:])

#         self.poses_rel = np.vstack(self.poses_list)
...
class data_partition():
    def __init__(self, opt, folder, skip_frames=10):
        ...
        self.sequence_length = opt.seq_len
        self.folder = folder
        self.skip_frames = skip_frames
        self.mov_path = self.root + f'frames/{self.folder}.mov'
        self.load_data()

    def load_data(self):
        N = self.skip_frames
        poses = np.loadtxt(self.root  + f'poses/{self.folder}.txt')[::6] # Images processed to 10fps
        imus = np.loadtxt(self.root + f'imus/{self.folder}.txt')
        timestamps = np.loadtxt(self.root + f"timestamps/{self.folder}.txt")[:,0][::6] # Images processed to 10fps

        image_dir = self.root + 'sequences/'
        png_paths = glob.glob('{}{}/images_2/*.png'.format(image_dir, self.folder))
        jpg_paths = glob.glob('{}{}/images_2/*.jpg'.format(image_dir, self.folder))
        self.img_paths = sorted(png_paths + jpg_paths)
        n_frames = len(self.img_paths)

        self.img_paths_list, self.poses_list, self.imus_list = [], [], []
        for i in range(n_frames-(self.sequence_length*N)):
            # Images and times
            start, end = timestamps[[i, i+(self.sequence_length*N)]]
            self.img_paths_list.append(self.img_paths[i:i + (self.sequence_length*N):N])
            # Poses
            pose_samples = poses[i:i + (self.sequence_length)*N:N]
            pose_samples = np.stack([np.concatenate((s.reshape(3,4), np.array([[0, 0, 0, 1]])), 0) for s in pose_samples])
            self.poses_list.append(get_relative_pose_sequence(pose_samples))
            # IMU samples
            imu_samples = imus[(imus[:,0] >= start) & (imus[:,0] <= end)][:,1:]
            self.imus_list.append(imu_samples)
    ...
    def __getitem__(self, i):
        ...
        image_sequence = []
        for img_path in image_path_sequence:
            img_as_img = np.transpose(np.asarray(Image.open(img_path)), (1,0,2))
            img_as_img = TF.resize(img_as_img, size=(self.opt.img_h, self.opt.img_w))
            img_as_tensor = TF.to_tensor(img_as_img) - 0.5
            img_as_tensor = img_as_tensor.unsqueeze(0)
            image_sequence.append(img_as_tensor)
    ...
    def extract_specific_frames(self, video_path, frame_indices):
        # Open the video file
        video = cv2.VideoCapture(video_path)
        frames = []
        for idx in frame_indices:
            # Set the frame number to the next index in frame_indices
            video.set(cv2.CAP_PROP_POS_FRAMES, idx)
            ret, frame = video.read()  # Read the frame
            if ret:  # If the frame was read successfully
                frame = np.transpose(frame, (2, 0, 1))
                frames.append(frame)
            else:  # If there was an error reading the frame
...
class KITTI_tester():
    def __init__(self, args):
        ...
        for seq in args.val_seq:
            self.dataloader.append(data_partition(args, seq, skip_frames=args.skip_frames))
...
class Vanilla_KITTI_tester():
    def __init__(self, args):
        super(Vanilla_KITTI_tester, self).__init__()
    ...
    def test_one_path(self, net, df, num_gpu=1, p=0.5):
        ...
        for i, (image_seq, imu_seq, gt_seq) in tqdm(enumerate(df), total=len(df), smoothing=0.9):  
            ...
            i_in = imu_seq.unsqueeze(0).repeat(num_gpu,1,1).cuda()
            with torch.no_grad():
                out = net(x_in, i_in, is_first=(i==0), hc=hc)
                pose = out[0]
                hc = out[-1]
            pose_list.append(pose[0,:,:].detach().cpu().numpy())
        pose_est = np.vstack(pose_list)
    ...
    def eval(self, net, num_gpu=1):
        ...
        for i, seq in enumerate(self.args.val_seq):
            ...
            pose_est_global, pose_gt_global, t_rel, r_rel, t_rmse, r_rmse, usage, speed = kitti_eval(pose_est, None, self.dataloader[i].poses_rel)
            dec_est = np.ones_like(speed[2:])
            prob_est = dec_est
            self.est.append({'pose_est_global':pose_est_global, 'pose_gt_global':pose_gt_global, 'decs':dec_est, 'probs':prob_est, 'speed':speed})
            self.errors.append({'t_rel':t_rel, 'r_rel':r_rel, 't_rmse':t_rmse, 'r_rmse':r_rmse, 'usage':usage})
...
def kitti_eval(pose_est, dec_est, pose_gt):
    ...
    # Convert the relative se3 poses to relative SE3
    #pose_est = np.stack([se3_exp(pose) for pose in pose_est])
    #pose_gt = np.stack([se3_exp(pose) for pose in pose_gt])

    # Transfer to 3x4 pose matrix, and get the global poses.
