...
def get_relative_pose_6DoF(Rt1, Rt2):
    ...
    # Transform SE3 to se3, [4x4] -> [6]
    #pose_rel = se3_log(Rt_rel)
...
def pose_accu(Rt_pre, R_rel):
    ...
    The R_rel has already been transformed back to SE3
    '''
...
def get_relative_pose_sequence(poses_abs):
    poses_rel = []
    for i in range(len(poses_abs)-1):
        poses_rel.append(get_relative_pose_6DoF(poses_abs[i], poses_abs[i+1]))
    return np.vstack(poses_rel)
...
def se3_log(T):
    ...
    Returns the 6 essential elements: 3 for rotation and 3 for translation.
    """
    ...
    A = np.eye(3) - omega_hat / 2 + (1 / theta**2) * (1 - theta / (2 * np.tan(theta / 2))) * np.dot(omega_hat, omega_hat)
    v = np.dot(A, t)
...
def se3_exp(xi):
    ...
    Accepts the 6 essential elements: 3 for rotation and 3 for translation.
    """
    v = xi[:3]
    omega = xi[3:]
    ...
    R = np.eye(3) + (np.sin(theta) / theta) * omega_hat + ((1 - np.cos(theta)) / theta**2) * np.dot(omega_hat, omega_hat)

    # Compute the translation component
    A = np.eye(3) + (1 - np.cos(theta)) / theta**2 * omega_hat + (theta - np.sin(theta)) / (theta**3) * np.dot(omega_hat, omega_hat)
    t = np.dot(A, v)
